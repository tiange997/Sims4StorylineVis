...
(function addExportButton() {
  ...
  btn.onclick = function() {
    ...
    // Clone SVG to avoid mutations
    const clone = svgElem.cloneNode(true);
    // Remove any transforms for export
    clone.removeAttribute('style');
    // Set width/height attributes for export
    clone.setAttribute('width', svgElem.viewBox.baseVal.width || svgElem.width.baseVal.value || 14000);
    clone.setAttribute('height', svgElem.viewBox.baseVal.height || svgElem.height.baseVal.value || 1200);

    // Serialise SVG
    const serializer = new XMLSerializer();
    let svgString = serializer.serializeToString(clone);

    // Fix for xmlns if missing
    if (!svgString.match(/^<svg[^>]+xmlns="http:\/\/www\.w3\.org\/2000\/svg"/)) {
      svgString = svgString.replace(/^<svg/, '<svg xmlns="http://www.w3.org/2000/svg"');
    }
    // Fix for xlink if missing
    ...
    img.onload = function() {
      ...
      a.download = 'storyline_export.png';
      a.href = canvas.toDataURL('image/png');
      a.click();
    };
    img.onerror = function() {
      alert('Failed to render SVG for export. Try zooming out or refreshing.');
    };
    img.src = imageSrc;
  };
})();
